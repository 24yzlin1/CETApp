...
async function callLlm(history: ChatMessage[]): Promise<ChatMessage> {
  ...
  return ["assistant", response !== undefined ? response.choices[0].message.content : error];
}

async function updateMemory(previous: string, sessions: ChatSessionData[]): Promise<string> {
  const messages = sessions.flatMap((x) => x[3]).filter((x) => x[0] !== "system");
  const result = await callLlm([["system", "合并旧记忆与新对话，精简提炼核心信息，去冗余、留关键设定，只输出压缩后的新记忆。旧记忆：" + previous], ...messages]);
  return result[1];
}